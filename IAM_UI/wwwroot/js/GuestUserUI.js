...
$(document).ready(function () {
    $('#dynamicTable_GuestUser').DataTable({
        "pageLength": 10
    });
...
$(document).ready(function () {
    ...
    function validatePAN(pan) {
        // Regular expression to validate PAN
        var panRegex = /^[A-Z]{5}[0-9]{4}[A-Z]{1}$/;
        return panRegex.test(pan);
    }

    $("#PAN").on("keypress input", function (e) {
        var pan = $(this).val().toUpperCase();
        $(this).val(pan);
        var isValidPAN = validatePAN(pan);
...
function Add() {
    ...
    if (Middle_Name == "" & Last_Name != "" & First_Name != "") {
...
function generatePassword() {
    ...
    //// Convert mobile number to a substring with only the first four digits
    //const mobileDigits = mobile.match(/\d/g).join('').substring(0, 4);
    // Convert mobile number to a substring with only the Last four digits
    const mobileDigits = mobile.match(/\d/g).join('').substr(-4);
    //// Convert mobile number to a substring with only the Middle four digits
    //const mobileDigits = mobile.match(/\d/g).join('').substring(3, 7);

    // Combine the first Four characters of the name with the first four digits of the mobile number
    password = name.substring(0, Math.min(name.length, 4)) + mobileDigits;
    /*alert(password);*/
  /*  Swal.fire("Thank You!!!", `This will be your password: ${password}`, "info");*/
    Swal.fire({
        title: `This will be your password: ${password}`, 
        icon: "success",
        showClass: {
            popup: `
      animate__animated
...
//$('#btnSaveGroupHeadPersonnel').submit(function (e) {
//    var panRegex = /^[A-Z]{5}[0-9]{4}[A-Z]{1}$/;
//    if (!panRegex.test($("#PAN").val())) {
//        swal.fire("Oppss!!!", "Please Insert Correct PAN No", "error");
//        return false;
//    }
//});
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (d[0].mailBody != null) {
                        swal.fire("Thank You!!!", "Email send to Guest User!!", "success");
                    }
                    else {
                        $("#email-loader").fadeOut();
...
function TagDelete(GroupHead_ID) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.getJSON("/Authorization/DeleteGuestUserID",
                {
                    id: GroupHead_ID
                },
                function (data) {
...
$(function () {
    $('#btnSaveGroupHeadPersonnel').on('click', function (e) {
        ...
        $.post('/Authorization/SaveGuestUser', formData, function (response) {
            Swal.fire({
                icon: 'success',
